...
if "%choice%"=="21" (
    ...
    curl -X POST %API_URL%/compile -H "Content-Type: application/json" -d "{\"model_path\": \"%BASE_DIR%/raw/resnet50.pt\", \"result_dir\": \"%BASE_DIR%/compiled\", \"target\": \"tensorrt\", \"options\": {\"optimization\": {\"fp16\": true}}}"
)
if "%choice%"=="22" (
    echo [测试22] TensorRT编译（从 ONNX 文件）
    curl -X POST %API_URL%/compile -H "Content-Type: application/json" -d "{\"model_path\": \"%BASE_DIR%/optimized/resnet.onnx\", \"result_dir\": \"%BASE_DIR%/compiled\", \"target\": \"tensorrt\", \"options\": {\"optimization\": {\"fp16\": true, \"workspace_size\": 4096}}}"
)
if "%choice%"=="23" (
    echo [测试23] Ascend编译（从原始 .pt 文件）
    curl -X POST %API_URL%/compile -H "Content-Type: application/json" -d "{\"model_path\": \"%BASE_DIR%/raw/resnet50.pt\", \"result_dir\": \"%BASE_DIR%/compiled\", \"target\": \"ascend\", \"options\": {\"device\": \"Ascend 310\", \"input_shape\": \"1,3,224,224\"}}"
)
if "%choice%"=="24" (
    echo [测试24] Ascend编译（从 ONNX 文件）
    curl -X POST %API_URL%/compile -H "Content-Type: application/json" -d "{\"model_path\": \"%BASE_DIR%/optimized/resnet.onnx\", \"result_dir\": \"%BASE_DIR%/compiled\", \"target\": \"ascend\", \"options\": {\"device\": \"Ascend 310\", \"input_shape\": \"1,3,224,224\", \"input_format\": \"NCHW\"}}"
)
